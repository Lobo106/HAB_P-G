...
let id = muyImportante.length + importante.length + normal.length;
...
function cargarListaMuyImportante(data) {
    data.forEach(function (i) {
        generarLiMuyImportante(i.tarea, i.time, i.id)
    })
}
...
function cargarListaImportante(data) {
    data.forEach(function (i) {
        generarLiImportante(i.tarea, i.time, i.id)
    })
}
...
function cargarListaNormal(data) {
    data.forEach(function (i) {
        generarLiNormal(i.tarea, i.time, i.id)
    })
}
...
function submitForm(e) {
    ...
    if (orden === "muyImportante") {
        ...
        generarLiMuyImportante(taskText, now, id, tic)
        ...
        //Y llama a la funcion que crea la tarea con la info que se ha metido en el form
        generarLiImportante(taskText, now, id, tic);
        //Guardamos el array en localStorage
        importante.push({
            tarea: taskText,
            time: now,
        ...
        //Y llama a la funcion que crea la tarea con la info que se ha metido en el form
        generarLiNormal(taskText, now, id, tic);
        //Guardamos el array en localStorage
        normal.push({
            tarea: taskText,
            time: now,
...
function deleteTask() {
    ...
    terminado.parentNode.parentNode.removeChild(terminado.parentNode);
...
listaMuyImportante.addEventListener("click", function (tarea) {
    ...
    if (element.checked) {
        element.classList.add("terminada")
        element.parentNode.classList.add("tachada")
        muyImportante[(element.parentNode.id) - 1].tic = "checked";
saveList();
    }
    else {
        element.classList.remove("terminada")
        element.parentNode.classList.remove("tachada")
        muyImportante[(element.parentNode.id) - 1].tic = "";
        saveList();
    }
});
listaImportante.addEventListener("click", function (tarea) {
    ...
    if (element.checked) {
        element.classList.add("terminada")
        element.parentNode.classList.add("tachada")
        importante[(element.parentNode.id) - 1].tic = "checked";
saveList();
    }
    else {
        element.classList.remove("terminada")
        element.parentNode.classList.remove("tachada")
        importante[(element.parentNode.id) - 1].tic = "";
        saveList();
    }
});
listaNormal.addEventListener("click", function (tarea) {
    ...
    if (element.checked) {
        element.classList.add("terminada")
        element.parentNode.classList.add("tachada")
        normal[element.parentNode.id].tic = "checked";
        saveList();
    }
    else {
        element.classList.remove("terminada")
        element.parentNode.classList.remove("terminada")
        normal[(element.parentNode.id) - 1].tic = "";
        saveList();
    }
});